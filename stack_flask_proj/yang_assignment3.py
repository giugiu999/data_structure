    ...
    while loop:
        ...
        while source_loop:
            ...
            if source.lower() == 'exit':
                ...
                raise Exception
            source = int(source)-1
            statement = valid_source(flasks,source)
            if statement != "":
                move_cursor(5,0)
                print(ANSI['CLEARLINE'],end = "")
                print(statement,end = "")
            ...
            while des_loop:
                ...
                statement = valid_destination(flasks,destination)
                if statement != "":
                    move_cursor(5,0)
                    print(ANSI['CLEARLINE'],end = '')
                    print(statement,end='')
            ...
            remove(flasks,source,destination)
            move_cursor(6,0)
            if win(flasks):
                display_flasks(flasks,source,destination)
                if len(flasks) == 4:
        ...
        #         move_cursor(5,0)
        #         print(ANSI['CLEARLINE'],end='')
        #         print("Invalid input. Try again111.",end='')
        #     else:
        #         move_cursor(6,0)
        #         display_flasks(flasks,source,destination)
        #         loop = False
...
def valid_source(flasks,source):
    """
    judge whether source is valid
    """
    if source<0 or source>=len(flasks):
        return 'Invalid input. Try again.'
    if flasks[source].is_sealed() or flasks[source].isEmpty():
        return 'Cannot pour from that flask. Try again.'
    return ''

def valid_destination(flasks,destination):
    """
    judge whether destination is valid
    """
    if destination<0 or destination>=len(flasks):
        return 'Invalid input. Try again.'
    if flasks[destination].is_sealed() or flasks[destination].isFull():
        return 'Cannot pour from that flask. Try again.'
    return ''
